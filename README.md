...
- 连接测试、取消、可配置的生图超时（默认 10 分钟，范围 1–60 分钟）、Base64/图片格式/响应大小校验（取消仅在图像 HTTP 请求阶段可用）
...
扩展设置中的 **生图超时时间（分钟）** 默认是 10 分钟，可设置为 1–60 分钟。该计时
只覆盖 `/v1/images/generations` 请求，不包含前置的聊天上下文提示词整理和后续图片上传。

达到扩展超时后会明确显示“图片生成超过 N 分钟”，手动点击取消则只显示“图片生成已取消”。
如果提高扩展超时后仍提前失败，还要检查 Sub2API、Nginx/CDN 或其他反向代理的读取超时；
例如扩展设为 10 分钟时，代理读取超时应略大于 600 秒。

浏览器中止等待不保证上游生成任务同步停止，任务仍可能继续执行或消耗额度。超时后先查看
Sub2API 日志和浏览器 Network 状态，不要立即连续重试。
